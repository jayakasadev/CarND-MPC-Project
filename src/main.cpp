...
/**
 * Method for transforming the map coordinates to the car coordinates
 *
 * @param ptsx
 * @param ptsy
 */
void transformToCarCoordinates(vector<double> &ptsx, vector<double> &ptsy, double px, double py, double psi){
    for(unsigned int a = 0; a < ptsx.size(); a++) {
        double dx = ptsx[a] - px;
        double dy = ptsy[a] - py;
        ptsx[a] = dx * cos(-psi) - dy * sin(-psi);
        ptsy[a] = dx * sin(-psi) + dy * cos(-psi);
    }
}
        ...
        if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
            ...
            if (s != "") {
                ...
                if (event == "telemetry") {
                    ...
                    transformToCarCoordinates(ptsx, ptsy, px, py, psi);

                    /*
                     * Fitting a 3rd order polynomial to the above x and y coordinates
                     *
                     * The ptsx and ptsy vectors must be mapped from std::vector to Eigen::Vector first
                     */
                    Eigen::Map<Eigen::VectorXd> x_vector(ptsx.data(), ptsx.size());
                    Eigen::Map<Eigen::VectorXd> y_vector(ptsy.data(), ptsy.size());
                    ...
                    double steer_value = -results[0]/ deg2rad(25); // convert to [-1..1] range

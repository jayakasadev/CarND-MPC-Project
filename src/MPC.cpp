 ...
 *
 * It was obtained by measuring the radius formed by running the vehicle in the simulator around in a circle with a
 * constant steering angle and velocity on a flat terrain.
 *
 * Lf was tuned until the the radius formed by the simulating the model presented in the classroom matched the previous
 * radius.
 *
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
     ...
     * For example: If the state is a 4 element vector, the actuators is a 2 element vector and there are 10 timesteps.
    ...
    /*
     * NOTE: Setting sparse to true allows the solver to take advantage of sparse routines, this makes the computation
     * MUCH FASTER. If you can uncomment 1 of these and see if it makes a difference or not but if you uncomment both
     * the computation time should go up in orders of magnitude.
     */
    ...
    for(int a = 0; a < solution.x.size(); a++){
    ...
    */
    ...
    // load the last N-1 points into the output block to display
    for(unsigned int a = 1; a < N; a++) {
        x_vals.push_back(solution.x[x_start + a]);
        y_vals.push_back(solution.x[y_start + a]);
    }
